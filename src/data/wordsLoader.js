export async function loadWords() {
  const cached = localStorage.getItem('words_he_cache_v2')
  if (cached) {
    try {
  ...
  try {
    ...
    // Cache a copy
    localStorage.setItem('words_he_cache_v2', JSON.stringify(data))
    return data
  } catch (e) {
...
export async function loadEnglishWords() {
  const cached = localStorage.getItem('words_en_cache_v2')
  if (cached) {
    try {
  ...
  try {
    ...
    const data = await resp.json()
    localStorage.setItem('words_en_cache_v2', JSON.stringify(data))
    return data
  } catch (e) {
